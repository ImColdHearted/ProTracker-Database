...
namespace Foot_Tracker.Tracking
{
    public static class EncounterDetector
    {
        ...
        /// <param name="looksLikeBattleTitle">True if the OCR'd title text contains
        /// "VS" - every real battle (wild encounter or boss) has this in its title,
        /// so this distinguishes "still trying to identify a real battle" from
        /// "this probably isn't a battle at all" (BattleWindowLocator's generic
        /// dark-bar heuristic can false-positive on other dark UI panels, e.g. NPC
        /// dialogue boxes - confirmed via a real tester screenshot/log). Lets the
        /// caller avoid showing a misleading "identifying Pokemon" status on
        /// screens that were never a battle in the first place.</param>
            ...
            out bool looksLikeBattleTitle)
        {
            ...
            looksLikeBattleTitle =
                cleaned.Contains("vs", StringComparison.OrdinalIgnoreCase);
